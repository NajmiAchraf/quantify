...
class BucketBrigadeRead(BucketBrigadeBase):
    """
    ...
    def construct_circuit(self) -> None:
        ...
        read_memory_moments = self.wiring_read_memory()

        # Decompose the circuits with appropriate decomposition types
        memory_read_decomposed = self.decompose_parallelize_toffoli(
            read_memory_moments,
            self.decomp_scenario.dec_mem_read,
            [0, 1, 2],
        )
