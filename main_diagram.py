...
MODULES = {
    ...
    "qram/circuit/assessment.py\nQRAMCircuitAssessment Class": {
        "description": "Generates assessment reports for QRAM Circuits",
        "color": COLOR_MODULE_CLASSES_INHERITANCE,
        "type": "Class Inheritance",
    },
    ...
    "main_bilan.py\nMainAssessment": {
        "description": "Executes QRAMCircuitAssessment",
        "color": COLOR_MODULE_MAIN,
        "type": "Main",
    },
    "main_stress.py\nMainStress": {
        ...
        for module, attrs in MODULES.items():
            if attrs["type"] == module_type:
                sub.node(
                    ...
                    fontcolor=(
                        "white" if attrs["color"] not in ["red", "gold"] else "black"
                    ),
                    shape="box",
                    fontsize="17",
                )
...
def create_diagram(engine="twopi", fmt="pdf", output_name="main_diagram"):
    ...
    dot = Digraph(
        comment="QRAM Circuit Module Dependencies Diagram", format=fmt, engine=engine
    )
    dot.attr(
        rankdir="LR",
        splines="ortho",
    ...
    add_cluster(
        dot,
        "classes_inheritance_composition",
    ...
    dependencies = [
        ...
        # Utils
        ("utils/arg_parser.py", "qram/circuit/core.py\nQRAMCircuitCore Class", "Utils"),
        ("utils/arg_parser.py", "main_experiments.py\nMainExperiments", "Utils"),
        ("utils/arg_parser.py", "main_bilan.py\nMainAssessment", "Utils"),
        ("utils/arg_parser.py", "main_stress.py\nMainStress", "Utils"),
        (
            "utils/print_utils.py",
            "qram/circuit/core.py\nQRAMCircuitCore Class",
            "Utils",
    ...
    for src, dst, relation in dependencies:
        relation_attrs = COLOR_RELATIONS.get(
            relation, {"color": "white", "style": "solid"}
        )
        dot.edge(
            src,
            dst,
