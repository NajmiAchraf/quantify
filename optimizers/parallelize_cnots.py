...
class ParallelizeCNOTSToLeft(InvariantCheckOptimizer):
    ...
    def optimization_at(self, circuit, index, op):
        ...
        while can_still_search_left:
            ...
            for qub_idx, qub in enumerate(op.qubits[1:], 1):
                tmp_tmp = circuit.prev_moment_operating_on([qub], p_idx_control)
                if tmp_tmp is None:
                    tmp_tmp = -1
            ...
            can_still_search_left = (
                can_still_search_left
                and ctu.is_controlled_parallel_x(potential_prev_op)
            )
            ...
            can_still_search_left = can_still_search_left and ctu.has_control_qubit(
                potential_prev_op, op.qubits[0]
            )
        ...
        current_controlled_op = self.merge_controlled_parallel_x(
            op, current_controlled_op
        )

        self.update_operation(circuit, p_idx_control, prev_op, current_controlled_op)
    ...
    def merge_controlled_parallel_x(self, op1, op2):
        ...
        if (
            ...
            raise ValueError("Control qubits must be the same for both operations")
        # if len(op1.controls) != len(op2.controls) or op1.controls[0] != op2.controls[0]:
        #     raise ValueError("Control qubits must be the same for both operations")
    ...
    def cnot_to_controlled_parallel_x(self, circuit, index, operation):
        ...
        current_controlled_op = cirq.ControlledOperation([operation.qubits[0]], p_op)
        ...
        circuit.insert(
            index + 1, current_controlled_op, strategy=cirq.InsertStrategy.INLINE
        )
...
#
# sub_gate = cirq.ops.GateOperation(cirq.ops.X, qubits)
#
