...
def test_recursive_adder():
    ...
    for constant in [0, 2, 5, 7]:
        ...
        for i in list_of_possible_integers:
            ...
            assert result.final_state_vector[i + constant] == 1
            intial_state[i] = 0
...
def test_controlled_recursive_adder():
    ...
    for constant in [0, 2, 5, 7]:
        circuit = ShorRecursiveAdder(
            A, constant, g, control
        ...
        list_of_possible_integers = [
            i for i in range(index_shift, index_shift + 2**n - constant)
        ]
        for i in list_of_possible_integers:
            intial_state[i] = 1
            intial_state = np.array(intial_state, dtype=np.complex64)
