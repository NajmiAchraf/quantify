...
def test_dec_NO_DECOMP():
    ...
    moments = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.NO_DECOMP, qubits=qubits
    ).decomposition()
    ...
    for i in range(8):
        ...
        if i in [6, 7]:
            temp[6] = 1 - temp[6]
            temp[-1] = 1 - temp[-1]
        assert np.array_equal(np.array(result.final_state_vector), temp)
...
def test_dec_ZERO_ANCILLA_TDEPTH_3():
    ...
    qubits = [cirq.NamedQubit(str(i)) for i in range(3)]
    moments = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.ZERO_ANCILLA_TDEPTH_3, qubits=qubits
    ).decomposition()
    circuit.append(moments)
    measurements = [cirq.measure(qubits[i]) for i in range(len(qubits))]
    circuit.append(measurements)
    ...
    for i in range(8):
        ...
        if i in [6, 7]:
            temp[6] = 1 - temp[6]
            temp[-1] = 1 - temp[-1]
        assert np.array_equal(np.array(np.around(result.final_state_vector)), temp)
...
def test_dec_ONE_ANCILLA_TDEPTH_2():
    ...
    dec = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.ONE_ANCILLA_TDEPTH_2, qubits=qubits
    )
    ...
    for i in range(8):
        initial_state[i] = 1
        result = simulator.simulate(
            circuit, qubit_order=all_qubits, initial_state=initial_state
        )
        temp = copy.deepcopy(
            initial_state
        )  # temp is supposed to have the expected result of a tofjoli
...
def test_dec_FOUR_ANCILLA_TDEPTH_1_A():
    ...
    qubits = [cirq.NamedQubit(str(i)) for i in range(3)]
    dec = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.FOUR_ANCILLA_TDEPTH_1_A, qubits=qubits
    )
    moments = dec.decomposition()
    circuit.append(moments)
    simulator = cirq.Simulator()
...
def test_dec_FOUR_ANCILLA_TDEPTH_1_B():
    ...
    qubits = [cirq.NamedQubit(str(i)) for i in range(3)]
    dec = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.FOUR_ANCILLA_TDEPTH_1_B, qubits=qubits
    )
    moments = dec.decomposition()
    circuit.append(moments)
    simulator = cirq.Simulator()
...
def test_dec_ZERO_ANCILLA_CNOT_3():
    ...
    qubits = [cirq.NamedQubit(str(i)) for i in range(3)]
    moments = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.ZERO_ANCILLA_CNOT_3, qubits=qubits
    ).decomposition()
    circuit.append(moments)
    measurements = [cirq.measure(qubits[i]) for i in range(len(qubits))]
    circuit.append(measurements)
    ...
    for i in range(8):
        ...
        assert np.array_equal(
            np.array(np.abs(np.around(result.final_state_vector))), temp
        )
        initial_state[i] = 0
...
def test_dec_ZERO_ANCILLA_CNOT_4():
    ...
    qubits = [cirq.NamedQubit(str(i)) for i in range(3)]
    moments = td.ToffoliDecomposition(
        decomposition_type=td.ToffoliDecompType.ZERO_ANCILLA_CNOT_4, qubits=qubits
    ).decomposition()
    circuit.append(moments)
    measurements = [cirq.measure(qubits[i]) for i in range(len(qubits))]
    circuit.append(measurements)
